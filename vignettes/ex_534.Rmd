...
params = make_inits_L_basic(pars = params, L0)
```
...
params = make_inits_MYZ_GeRM(params, M0 = M0, G0 = G0, Y0 = Y0, Z0 = Z0, Upsilon0=Upsilon)
params = attach_inits_MYZ(params)
```
...
params = make_inits_X_SIS(params, X0)
params = attach_inits_X(params)
```
...
params = make_parameters_vc_null(params)
```
...
```{r}
out <- deSolve::ode(y = y0, times = 0:365, func = xDE_diffeqn, parms = params, method = "lsoda")
```
