...
To use `xde_setup_human` we must set the values of $Z$ that give us the correct EIR values, and we set these as $Zm$. The function call is to `Ztrace` 

```{r}
MYZo = list(f=0.3, q=0.95)
MYZo$Zm = with(c(MYZo,Xo), h/b/f/q*Hpop) 
```


```{r}
xde_setup_human("test_hMoI", "hMoI", HPop=Hpop, Xopts = Xo, MYZopts = MYZo) -> test_hMoI
```
