...
In GeRM, baseline parameters are determined by resource availability: the availability of blood hosts ($B$), the availability of aquatic habitats ($Q$), and the availability of sugar sources ($S$). To build models of blood feeding and transmission, we subdivide blood hosts into the availability of the pathogen's hosts ($W$), and other blood hosts ($O$). 
...
params = setup_forcing_MYZ_GeRM_basic(params, other=other, sugar=sugar, W=hosts, zeta=1, Q=habitat)
```
...
params = make_parameters_MYZ_GeRM_static(params, g, sigma, f, q, nu, eggsPerBatch, eip, calK, "ode")
```
...
colnames(out)[params$MYZpar$G_ix+1] <- paste0('G_', 1:params$nPatches)
