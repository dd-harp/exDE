...
```{r}
Xo = list(b=0.55, c=0.15, r=1/200, X0 = X)
Hpop = c(100, 500, 250)
```
...
```{r}
EIR <- with(Xo, diag(1/b, nStrata) %*% ((r*X0)/(Hpop-X0)))
MYZo = list(f=0.3, q=0.95)
MYZo$Zm = with(c(MYZo,Xo), EIR/f/q*Hpop) 
```


```{r}
xde_setup_human("test_SIS", "SIS", HPop=Hpop, Xopts = Xo, MYZopts = MYZo) -> test_SIS
```

```{r}
xde_solve(test_SIS, 365, 365) -> out2
