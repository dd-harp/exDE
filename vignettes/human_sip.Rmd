...
```{r}
EIR <- with(Xo, diag(1/b, nStrata) %*% diag(1/(1-rho)) %*% ((r*X0)/(Hpop-X0-P0)))
...
```{r}
xde_setup_human("test_SIP", "SIP", HPop=Hpop, Xopts = Xo, MYZopts = MYZo) -> test_SIP
```

```{r}
xde_solve(test_SIP, 365, 365) -> out2
