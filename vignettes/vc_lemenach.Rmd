...
with(pars$MYZpar,{
  ...
  colnames(out)[M_ix+1] <- paste0('M_', 1:pars$nPatches)
  colnames(out)[P_ix+1] <- paste0('P_', 1:pars$nPatches)
  colnames(out)[Y_ix+1] <- paste0('Y_', 1:pars$nPatches)
  colnames(out)[Z_ix+1] <- paste0('Z_', 1:pars$nPatches)
  colnames(out)[pars$Xpar$X_ix+1] <- paste0('X_', 1:pars$nStrata)
  
  out <- out[, -c(sigma_ix+1)]
  out <- out[, -c(g_ix+1)]
  out <- out[, -c(q_ix+1)]
  out <- out[, -c(f_ix+1)]
  out <- out[, -c(kappa_ix+1)]
  out <- out[, -c(Upsilon_ix+1)]
