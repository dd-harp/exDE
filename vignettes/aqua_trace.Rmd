...
# ODE
params$beta = beta
params = make_Lpar_trace(pars = params, Lambda = alpha)
params = make_MYZpar_basicM(pars = params, g = g, sigma = sigma, calK = calK, f = f, q = q, nu = nu, eggsPerBatch = eggsPerBatch)
params = make_MYZinits_basicM(params, M0=M_eq, P0=P_eq)
params = make_indices(params)
```

```{r}
y0 <- rep(0, params$max_ix)
y0[params$ix$MYZ$M_ix] <- M_eq
y0[params$ix$MYZ$P_ix] <- P_eq
...
```{r, out.width = "100%"}
colnames(out)[params$ix$MYZ$M_ix+1] <- paste0('M_', 1:params$nPatches)
colnames(out)[params$ix$MYZ$P_ix+1] <- paste0('P_', 1:params$nPatches)
